...
export default function NotFound() {
  ...
  return (
    <div className="flex flex-col min-h-screen relative z-[100] bg-surface text-on-surface">
        ...
        <div className="max-w-5xl w-full grid grid-cols-1 md:grid-cols-12 gap-12 items-center">
          ...
          <div className="md:col-span-7 relative order-2 md:order-1">
            ...
            <div className="absolute -top-10 -right-4 md:-right-12 z-20 glass-panel p-10 rounded-lg whisper-shadow transform rotate-3 border border-white/20">
              ...
              <div className="mt-4 text-xs tracking-widest uppercase font-bold text-tertiary opacity-60">Architectural Error</div>
            </div>
            
            {/* Decorative element */}
            <div className="absolute -bottom-6 -left-6 w-32 h-32 bg-tertiary-container/10 rounded-full blur-3xl"></div>
          </div>
          
          {/* Text Content Section */}
          <div className="md:col-span-5 flex flex-col space-y-8 order-1 md:order-2">
            <div className="space-y-4">
              <div className="inline-block px-4 py-1.5 rounded-full bg-secondary-fixed text-on-secondary-fixed-variant text-xs font-bold tracking-[0.1em] uppercase">
                  Lost in Sanctuary
              </div>
              ...
              <p className="text-lg md:text-xl text-on-surface/70 font-medium leading-relaxed">
                  The page you are looking for has moved or no longer exists. Let us guide you back to safety.
              </p>
            </div>
            
            <div className="flex flex-col sm:flex-row flex-wrap gap-4 pt-4">
              <Link href="/" className="inline-flex items-center justify-center px-8 py-4 bg-tertiary-container text-on-tertiary-container rounded-full font-bold hover:scale-105 transition-all duration-300 active:scale-95 shadow-lg shadow-tertiary-container/20">
                <Home className="mr-2 h-5 w-5" />
                Return to Home
              </Link>
              <Link href="/contact" className="inline-flex items-center justify-center px-8 py-4 border border-outline-variant bg-surface-container-lowest/50 backdrop-blur-sm text-primary rounded-full font-bold hover:bg-surface-container-low hover:scale-105 transition-all duration-300 active:scale-95">
