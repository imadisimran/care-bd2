export default function CourseStats() {
  ...
  return (
    <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
      {stats.map((stat, i) => (
        <div key={i} className="bg-base-100 backdrop-blur-xl p-8 rounded-lg whisper-shadow flex flex-col items-center text-center transition-all duration-300 hover:-translate-y-1">
          ...
          <div className="text-sm font-semibold text-neutral uppercase tracking-wider">{stat.label}</div>
        </div>
      ))}
    </section>
  );
