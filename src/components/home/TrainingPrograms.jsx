...
export default function TrainingPrograms() {
  ...
  return (
    <section className="py-section px-8 lg:px-16 bg-base-100">
      <div className="container mx-auto max-w-6xl">
        <div className="text-center mb-16 space-y-4">
          <h2 className="text-4xl lg:text-5xl font-bold tracking-tight text-primary">
            Professional <span className="text-success">Training Programs</span>
          </h2>
          <p className="text-neutral max-w-3xl mx-auto text-lg leading-relaxed">
            ...
            <div key={idx} className="service-card-glass rounded-[2rem] overflow-hidden flex flex-col group hover:shadow-2xl transition-all duration-500 border-white/60">
              <div className="relative h-64 overflow-hidden">
                ...
                <div className="absolute top-4 left-4 flex gap-2">
                  ...
                  <span className="px-3 py-1 bg-success text-white text-[10px] font-bold uppercase rounded-full">NSDA</span>
                </div>
                {prog.price && (
                  <div className="absolute top-4 right-4 bg-warning px-4 py-1 rounded-full">
                    <span className="text-[10px] font-bold text-base-content">{prog.price}</span>
                  </div>
                )}
              </div>
              <div className="p-8 flex-1 flex flex-col space-y-6">
                <div className="space-y-2">
                  <h3 className="text-2xl font-bold text-primary">{prog.title}</h3>
                  <div className="flex items-center gap-6 text-sm text-neutral/80 font-medium">
                ...
                <p className="text-neutral leading-relaxed">{prog.desc}</p>
                <div className="bg-base-200/50 p-4 rounded-xl flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-info/20 flex items-center justify-center text-primary">
                    <span className="material-symbols-outlined text-xl">person</span>
                  </div>
                  <div>
                    <div className="text-[10px] font-black uppercase tracking-widest text-neutral/60">Instructor</div>
                    <div className="text-sm font-bold text-primary">{prog.instructor}</div>
                  </div>
                </div>
                <button className="w-full bg-success text-white py-4 rounded-xl text-sm font-black uppercase tracking-[0.2em] hover:bg-success/90 hover:scale-[1.01] transition-all flex items-center justify-center gap-2">
        ...
        <div className="mt-16 text-center">
          <button className="border border-success text-success px-12 py-4 rounded-full text-sm font-black uppercase tracking-[0.2em] hover:bg-success hover:text-white transition-all duration-300 flex items-center gap-2 mx-auto">
